...
export async function POST(request: Request): Promise<Response> {
  if (process.env.ENABLE_STUDENTPORTAL_CMS !== 'true') {
    return Response.json({ error: 'Tipsinnsending er midlertidig deaktivert.' }, { status: 503 })
  }
  ...
  const campus = normalizeString(body.campus) || 'all'
  ...
  const payloadCreate = payload.create as unknown as (args: {
    collection: string
