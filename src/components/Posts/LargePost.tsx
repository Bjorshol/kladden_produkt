...
export const LargePost: React.FC<LargePostProps> = ({ post }) => {
  ...
  return (
    <Link href={`/posts/${post.slug}`} className="block focus:outline-none focus:ring-2 focus:ring-red-600">
      <div className="bg-white border border-gray-200 rounded-lg overflow-hidden hover:shadow-lg hover:border-gray-300 transition-all duration-200 w-full">
        {imageUrl && (
        ...
        <div className="p-3 md:p-4">
          {stikktittel && (
            <div className="text-xs md:text-sm uppercase tracking-wide text-gray-500 mb-2 truncate">
              {stikktittel}
            </div>
          )}
          <h2
            className="font-bold leading-tight mb-2 break-words text-neutral-900"
            style={{
